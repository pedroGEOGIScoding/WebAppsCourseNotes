...
7. **git branch** (checking in which branch you are working on at this moment)
    ...
    d) **git checkout <name of the branch you want to move on>** (to move to another branch)
    e) **git branch -D <name of the branch to delete>**
...
8. There are two commands to upload to the github repositoty the commits made. These two have to be executed by this order:
    ...
    b) second every time to upload commits remotely: **git push <connection name, which normally is names as origin> master**
...
10. **git pull <github url> branch** (download all files from the remote repo)
